...
import { getTimeEntries, startTimeEntry, stopTimeEntry, logJob, getActor } from '../features/offline/db'
...
export default function JobTimePanel({ jobId }: { jobId: string }) {
  ...
  async function clockIn() {
    try {
      ...
      await logJob(jobId, 'time_clock_in', `Clock in for ${date}`, getActor())
      setRows(await getTimeEntries(jobId))
    } catch (error: any) {
  ...
  async function clockOut() {
    try {
      const entry = await stopTimeEntry(jobId)
      await logJob(jobId, 'time_clock_out', `Clock out (${entry.date})`, getActor())
      setRows(await getTimeEntries(jobId))
    } catch (error: any) {
